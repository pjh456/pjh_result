...
{
    auto r = res::Result<int, std::string>::Ok(10).map([](int v) { return v * 2; });
    CHECK(r.is_ok());
    CHECK(r.unwrap() == 20);
}
...
{
    auto r = res::Result<int, std::string>::Err(std::string("e")).map([](int v) { return v * 2; });
    CHECK(r.is_err());
...
{
    bool called = false;
    auto r = res::Result<int, std::string>::Ok(3).map([&](int) { called = true; });
    CHECK(called);
    CHECK(r.is_ok());
}
...
TEST_CASE("and_then chains on Ok")
{
    auto r = res::Result<int, std::string>::Ok(3).and_then(
        [](int x) { return res::Result<int, std::string>::Ok(x + 100); });
    CHECK(r.unwrap() == 103);
}

TEST_CASE("and_then short-circuits on Err")
{
    auto r = res::Result<int, std::string>::Err(std::string("e")).and_then(
        [](int x) { return res::Result<int, std::string>::Ok(x + 100); });
    CHECK(r.is_err());
    CHECK(r.unwrap_err() == "e");
}
...
{
    auto r = res::Result<int, std::string>::Err(std::string("e")).or_else(
        [](const std::string &) { return res::Result<int, std::string>::Ok(0); });
    CHECK(r.is_ok());
    CHECK(r.unwrap() == 0);
}
...
{
    auto r = res::Result<int, std::string>::Err(std::string("e")).or_else(
        [](const std::string &e) { return res::Result<int, std::size_t>::Err(e.size()); });
    CHECK(r.is_err());
    CHECK(r.unwrap_err() == std::size_t{1});
}
