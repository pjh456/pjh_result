...
{
    auto r = rp::Result<void, std::string>::Ok();
    CHECK(r.is_ok());
...
{
    auto r = rp::Result<void, std::string>::Err(std::string("e"));
    CHECK(r.is_err());
...
{
    int seen = 0;
    auto r = rp::Result<void, std::string>::Ok().map([&]() { seen = 1; });
    CHECK(seen == 1);
    CHECK(r.is_ok());
}
...
{
    auto r = rp::Result<void, std::string>::Ok().map([]() { return 99; });
    CHECK(r.is_ok());
    CHECK(r.unwrap() == 99);
}
...
{
    auto r = rp::Result<void, std::string>::Ok().and_then(
        []() { return rp::Result<int, std::string>::Ok(7); });
    CHECK(r.unwrap() == 7);
}
